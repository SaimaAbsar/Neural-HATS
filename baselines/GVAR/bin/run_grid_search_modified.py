...
from datasets.saima.generate_6_ER import generate_timeseries
...
parser.add_argument('--experiment', type=str, default="new-dataset", help="Experiment to be performed (default: "
                                                                       "'lorenz96')")
...
if args.experiment == "fmri":
    args.p = 5
    args.T = 1200
    #print("p = 15, T = 200...")
    for i in range(args.num_sim):
        a_i = np.load('/Data/Netsim/sim5/gt_sim5.npy')  # update path to ground truth graph here
        a_i = np.transpose(a_i)
        data_i = np.loadtxt('/Data/Netsim/sim5/sim5_3.txt', delimiter=',')  # update path to data here
...
elif args.experiment == "new-dataset":  # for Neural-HATS data
    signed_structures = None
    for i in range(args.num_sim):
        a_i = np.load('/Data/A_6_ER_new.npy')   # update path to ground truth graph here
        a_i = np.transpose(a_i)
        data_i = generate_timeseries(500, 0.01)
        for j in range(data_i.shape[1]):
            data_i[:, j] = (data_i[:, j] - np.mean(data_i[:, j])) / np.std(data_i[:, j])
        datasets.append(data_i)
        structures.append(a_i)
...
if args.model == "var":
    ...
    for l in range(n_datasets):
        ...
        accs[l], bal_accs[l], precs[l], recs[l], f1[l] = eval_causal_structure_binary(a_true=a_l, a_pred=a_hat * 1.0)
        aurocs[l], auprcs[l] = eval_causal_structure(a_true=a_l, a_pred=-pvals)
        if signed_structures is not None:
            _, bal_accs_neg[l], __, ___, _ = eval_causal_structure_binary(a_true=(a_l_signed < 0) * 1.0,
                                                                       a_pred=(a_hat_signed < 0) * 1.0)
            _, bal_accs_pos[l], __, ___, _ = eval_causal_structure_binary(a_true=(a_l_signed > 0) * 1.0,
                                                                       a_pred=(a_hat_signed > 0) * 1.0)
    if signed_structures is None:
        print("Acc. = " + str(np.mean(accs)) + " ± " + str(np.std(accs)) +
              ...
              " ± " + str(np.std(recs)) + "; F1. = " + str(np.mean(f1)) + " ± " + str(np.std(f1)) + "; AUROC = " + str(np.mean(aurocs)) + " ± " + str(np.std(aurocs)) +
