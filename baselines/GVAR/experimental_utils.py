...
from utils import eval_causal_structure, eval_causal_structure_binary, my_results

from datetime import date

from training import training_procedure_trgc, training_procedure_stable
...
def run_grid_search(lambdas: np.ndarray, gammas: np.ndarray, datasets: list, structures: list, K: int,
    ...
    mean_f1 = np.zeros((len(lambdas), len(gammas)))
    sd_f1 = np.zeros((len(lambdas), len(gammas)))
    ...
    for i in range(len(lambdas)):
        ...
        for j in range(len(gammas)):
            ...
            for l in range(n_datasets):
                ...
                acc_l, bal_acc_l, prec_l, rec_l, f1_l = eval_causal_structure_binary(a_true=a_l, a_pred=a_hat_l)    #, diagonal=True)
                # added by Saima:
                accuracy, precision, recall, f1, shd = my_results(a_true=a_l, a_pred=a_hat_l)
                a_true=np.array(a_l) 
                a_pred=np.array(a_hat_l)
                print('A true: \n', a_true)
                print('A predicted: \n', a_pred)
                ...
                print('Recall: ', recall)
                print('F1-score: ', f1)
                auroc_l, auprc_l = eval_causal_structure(a_true=a_l, a_pred=a_hat_l_)   #, diagonal=True)
                ...
                print("Dataset #" + str(l + 1) + "; Acc.: " + str(np.round(acc_l, 4)) + "; Bal. Acc.: " +
                      ...
                      str(np.round(rec_l, 4)) + "; F1.: " + str(np.round(f1_l, 4)) + "; AUROC: " + str(np.round(auroc_l, 4)) + "; AUPRC: " +
            ...
            mean_f1[i, j] = np.mean(f1_ij)
            print("F1.         :" + str(mean_f1[i, j]))
            sd_f1[i, j] = np.std(f1_ij)
