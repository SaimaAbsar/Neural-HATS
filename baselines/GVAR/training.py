...
def run_epoch(epoch_num: int, model: nn.Module, optimiser: optim, predictors: np.ndarray, responses: np.ndarray,
    ...
    for i in range(len(batch_split)):
        ...
        # ===========================================================================================================================
        CI_table = np.loadtxt('/outputs/CI_table_ER1.txt', delimiter=',')    # update the path to the CI matrix here
        CI_table = np.transpose(CI_table)
        #print("CI_table:\n", CI_table)
        CI_loss = CI_test_Loss(coeffs, CI_table, device)  #   CI_table = M: conditional-independance testing to add constraint
        # ===========================================================================================================================
        ...
        loss = base_loss + lmbd * penalty + gamma * penalty_smooth + 0.5*CI_loss
