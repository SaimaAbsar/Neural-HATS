...
class Model(nn.Module):
    def __init__(self, batch_size, input_size, output_size, hidden_dim, n_layers, n):
        ...
        self.no_of_nodes = n
        adj = torch.load('A_pred.pt').to(device) 
        self.A = nn.Parameter(adj, requires_grad = True)
    ...
    def forward(self, x):         
        batch_size = x.size(0)
        # LSTM
        h_LSTM = torch.zeros((batch_size, self.hidden_dim, self.no_of_nodes)).to(device)
        self.hidden = self.init_hidden(batch_size)
        for j in range(self.no_of_nodes):
            lstm_out, (h_n,c_n)= self.lstm(torch.unsqueeze(x[:,:,j],2), self.hidden)
            h_LSTM[:, :, j] = h_n[-1,:,:]  #shape of h_LSTM = [batch, hidden_dim, no_of_nodes]
        ...
        #GCN
        alpha = 0.90 #parameter for balance of self-information
        ...
        h_out = (1-alpha) * AT_times_W3h + alpha * W1_h_A   #shape = [batch, no_of_nodes, self.gcn_lin_dim]  # weighted summation version  

        # MLP
        out_MLP = torch.empty((batch_size, self.no_of_nodes)).to(device)
        for k in range(self.no_of_nodes):
            ma = h_out[:, :, k]
            out_MLP[:,k] = torch.squeeze(self.fc(ma))
        return (out_MLP)
