...
def CI_test_Loss(weight, CI_table):
    ...
    mat = np.multiply(CI_table, weight)
...
def from_numpy_dynamic(  # pylint: disable=too-many-arguments
    ...
    w_est, a_est = _learn_dynamic_structure(
        X, Xlags, CI_table, bnds, lambda_w, lambda_a, max_iter, h_tol
    )
...
def _learn_dynamic_structure(
    ...
    def _func(wa_vec: np.ndarray) -> float:
        ...
        # ===========================================================================================================================
        CI_loss1 = CI_test_Loss(_a_mat, CI_table)  #   CI_table = M: conditional-independance testing to add constraint
        CI_loss2 = CI_test_Loss(_w_mat, CI_table)
        lambda_CI = 0.01
        # ============================================================================================================================
        return loss + 0.5 * rho * _h_value * _h_value + alpha * _h_value + l1_penalty + lambda_CI*CI_loss1 + lambda_CI*CI_loss2
